        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {templates.map((template: TemplateLayoutsWithSettings) => {
            ...
            return (
              <Card
                key={template.id}
                className={`${typeof selectedTemplate !== 'string' && selectedTemplate?.id === template.id ? 'border-2 border-blue-600' : ''} cursor-pointer hover:shadow-lg transition-all duration-200 group overflow-hidden relative`}
              ...
              >
                ...
                {typeof selectedTemplate !== 'string' && selectedTemplate?.id === template.id && (
                  <div className="absolute top-0 right-0 bg-blue-600 text-white px-2 py-1 rounded-bl-lg">
                    Selected
                  </div>
                )}
              </Card>
