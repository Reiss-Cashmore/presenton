...
def get_llm_provider():
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Invalid LLM provider. Please select one of: openai, google, anthropic, ollama, custom, openai-chatgpt",
        )
...
def is_openai_chatgpt_selected():
    return get_llm_provider() == LLMProvider.OPENAI_CHATGPT
...
def get_model():
    ...
    elif selected_llm == LLMProvider.OPENAI_CHATGPT:
        return get_chatgpt_model_env() or DEFAULT_CHATGPT_MODEL
    else:
        raise HTTPException(
            status_code=500,
            detail=f"Invalid LLM provider. Please select one of: openai, google, anthropic, ollama, custom, openai-chatgpt",
